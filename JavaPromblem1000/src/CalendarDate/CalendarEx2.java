...
public class CalendarEx2 {
	...
	public static void main(String[] args) {
		...
		// month의 경우 0 부터 시작하기 때문에 8월인 경우, 7 로 지정해야 한다.
		// date1.set(2015, Caledar.AUGUST, 15); 와 같이 할 수도 있다.
		date1.set(2015, 7,15);
		...
		System.out.println("date2은 " + toString(date2) +DAY_OF_WEEK[date2.get(Calendar.DAY_OF_WEEK)] 
				+ "요일입니다.");
		// 두 날짜간의 차이를 얻으려면, getTimeMillis() 천 분의 일초 단위로 변환해야 한다.
		long difference = 
				( (date2.getTimeInMillis() - date1.getTimeInMillis()  )/ 1000);
		System.out.println("그 날(date1) 부터 지금(date2) 까지 " + difference + "초가 지나갔습니다.");
